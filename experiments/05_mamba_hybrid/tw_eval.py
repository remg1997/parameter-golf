...
class Hyperparameters:
    ...
    # [05] Mamba hybrid: comma-separated layer indices to replace with Mamba blocks.
    # E.g., MAMBA_LAYER_INDICES="6" replaces layer 6's attention with a Mamba SSM.
    # Empty string => no replacement (falls back to pure transformer).
    mamba_layer_indices = os.environ.get('MAMBA_LAYER_INDICES', '')
...
# [05] Drop-in replacement for CausalSelfAttention using Mamba selective SSM.
# Conceptual contrast:
#   - Self-attention has O(N^2) compute and O(N^2) memory in sequence length.
#     Each token attends to all prior tokens via the QK^T similarity matrix.
#   - Mamba (Selective SSM, Gu & Dao 2023) has O(N) compute and O(1) memory in
#     seq length: each token reads from a fixed-size state h_t (~16 dims),
#     writes to it, and passes it forward. The "selectivity" comes from making
#     the state-update parameters input-dependent (A, B, C are functions of x_t),
#     letting the model decide what information to keep, drop, or emit.
#   - Math (simplified): h_t = A(x_t) * h_{t-1} + B(x_t) * x_t ; y_t = C(x_t) * h_t.
#     With diagonal A and structured discretization, the recurrence is computed
#     via a parallel scan in O(N log N) on GPU (fast enough to compete with attn).
#   - mamba_ssm provides the CUDA-fused selective_scan kernel; we just import it.
class MambaInner(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, seq, dim) — Mamba returns same shape.
        # Outer Block class adds the residual via attn_scale, just like with attn.
        return self.mamba(x)
...
class GPT(nn.Module):
    ...
    def __init__(self, h):
        ...
        # [05] Swap in Mamba blocks for selected layer indices, AFTER all attn-
        # specific setup (rope, xsa, parallel) so we can cleanly overwrite.
        # Skip indices that overlap with parallel_residual_start to keep things simple.
        if h.mamba_layer_indices:
            self._mamba_indices = [int(s) for s in h.mamba_layer_indices.split(',') if s.strip()]
            for idx in self._mamba_indices:
                if idx < 0 or idx >= h.num_layers:
                    continue
                self.blocks[idx].attn = MambaInner(h.model_dim, d_state=h.mamba_d_state, d_conv=h.mamba_d_conv, expand=h.mamba_expand)
                self.blocks[idx].parallel = False  # parallel residual logic is attn-specific
        else:
            self._mamba_indices = []
