...
class Hyperparameters:
    ...
    # [08] PPM mixer controls (eval-time only; training is unaffected)
    ppm_mixer_enabled = bool(int(os.environ.get('PPM_MIXER_ENABLED', '1')))
...
# [08] Byte-PPM mixer (PPM-D order-4 with strict-legal causal gate).
# Adapted from PR #1795 (OE-GOD). Compliance: cf gate is computed from the
# deepest context with data BEFORE looking up the observed byte's count, so
# the gate is purely a function of the prefix. Score-before-update on every byte.
def _ppm_mixture_bpb(tgt_np, lp_np, sp, O=4, H=0.9, L_=0.05, T=0.9):
    V = sp.vocab_size()
    piece_bytes = [None] * V
    piece_lens = np.zeros(V, dtype=np.int32)
    for i in range(V):
        p = sp.id_to_piece(i)
    ...
    plp = np.empty(N, dtype=np.float64)
    cf = np.empty(N, dtype=np.float64)
    LN256 = math.log(1 / 256)
    log_ = math.log
    h_ctx = b''
    for i in range(N):
        x = bs[i]
...
def eval_val_sliding(h, device, val_data, base_model, batch_seqs=32):
    ...
    # [08] capture per-token log-probs and target ids in scoring order for the PPM mixer
    tga_local = []
    lpa_local = []
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                byte_count += tb.sum()
                # [08] capture for PPM mixer (lp = -nll). Order is rank-local sliding order;
                # rank-major concatenation across ranks below restores global val order.
                tga_local.append(tgt.cpu().to(torch.int64))
                lpa_local.append((-scored_nll).cpu().to(torch.float64))
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
    val_loss, val_bpb = _loss_bpb(loss_sum, token_count, byte_count)
    # [08] gather (tga, lpa) across ranks to rank 0 and run PPM mixer
    if getattr(h, 'ppm_mixer_enabled', False):
        tga_local_cat = torch.cat(tga_local) if tga_local else torch.zeros(0, dtype=torch.int64)
        lpa_local_cat = torch.cat(lpa_local) if lpa_local else torch.zeros(0, dtype=torch.float64)
        if dist.is_available() and dist.is_initialized():
            local_size = torch.tensor([tga_local_cat.numel()], dtype=torch.int64, device=device)
            sizes = [torch.zeros(1, dtype=torch.int64, device=device) for _ in range(h.world_size)]
            dist.all_gather(sizes, local_size)
            sizes_list = [int(s.item()) for s in sizes]
            max_size = max(sizes_list) if sizes_list else 0
            tga_pad = torch.zeros(max_size, dtype=torch.int64, device=device)
            lpa_pad = torch.zeros(max_size, dtype=torch.float64, device=device)
            tga_pad[:tga_local_cat.numel()] = tga_local_cat.to(device)
            lpa_pad[:lpa_local_cat.numel()] = lpa_local_cat.to(device)
            if h.rank == 0:
                gather_t = [torch.zeros(max_size, dtype=torch.int64, device=device) for _ in range(h.world_size)]
                gather_l = [torch.zeros(max_size, dtype=torch.float64, device=device) for _ in range(h.world_size)]
            else:
                gather_t = None
                gather_l = None
            dist.gather(tga_pad, gather_t, dst=0)
            dist.gather(lpa_pad, gather_l, dst=0)
            if h.rank == 0:
